...
from app.schemas.borrow import BorrowCreate, BorrowDecision, BorrowOut
...
async def request_borrow(payload: BorrowCreate, current_user: dict = Depends(require_roles("student"))) -> BorrowOut:
    ...
    active = await db.borrows.count_documents({"student_id": current_user["id"], "status": {"$in": ["requested", "borrowed", "overdue"]}})
    ...
    already_requested = await db.borrows.find_one({"student_id": current_user["id"], "book_id": payload.book_id, "status": {"$in": ["requested", "borrowed", "overdue"]}})
...
async def approve_borrow(borrow_id: str, payload: BorrowDecision, current_user: dict = Depends(require_roles("librarian", "admin"))) -> BorrowOut:
    ...
    updated = await db.borrows.find_one_and_update({"_id": borrow["_id"], "status": "requested"}, {"$set": {"status": "borrowed", "approved_at": now, "issued_at": now, "due_date": due_date, "note": payload.note}}, return_document=ReturnDocument.AFTER)
    if updated is None:
    ...
    await create_notification(updated["student_id"], "Borrow request approved", f"Your request for {book['title']} was approved. It is due on {due_date.date().isoformat()}.", "borrow_approved")
...
async def fine_status(current_user: dict = Depends(require_roles("student"))) -> dict:
    ...
    return {"fine_amount": round(outstanding, 2), "currency": "USD"}
